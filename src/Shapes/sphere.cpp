...
namespace Shapes
{
        ...
        : _position{pos}, _radius{radius}
    {
        ...
        for (uint32_t lat = 1; lat < stacks; ++lat)
        {
            ...
            float h_angle = 0;
            for (uint32_t lon = 1; lon <= sectors; ++lon)
            {
                ...
                _vertices.emplace_back(res, tx, ty);
    ...
    void Sphere::setIndices(std::vector<uint32_t>& ind, uint32_t stacks, uint32_t lat, uint32_t sectors, uint32_t lon){
        ...
        if(lat == 1){
            ...
            ind.emplace_back(sectors*(lat-2) + lon - 1);
            ind.emplace_back(sectors*(lat-1) + lon);
            ind.emplace_back(sectors*(lat-1) + lon - 1);

            ind.emplace_back(sectors*(lat-2) + lon - 1);
            ind.emplace_back(sectors*(lat-2) + lon);
            ind.emplace_back(sectors*(lat-1) + lon);

            if(lon == sectors){
                ind.emplace_back(sectors*(lat-1));
                ind.emplace_back(sectors*(lat-1) + 1);
                ind.emplace_back(sectors*lat);
            
                ind.emplace_back(sectors*(lat-1));
                ind.emplace_back(sectors*(lat-2) + 1);
                ind.emplace_back(sectors*(lat-1) + 1);
            }
        }
    }
